...
class LoanAdmin(generic.BOAdmin):
    ...
    extra_buttons = [{'href':'pay','title':'pay'}]
...
class ReimbursementAdmin(generic.BOAdmin):
    ...
    fieldsets = [
        ...
        (('fico'),{'fields':[('org',),('loan',),('logout_amount','pay_amount',)],'classes': ['collapse']})
    ]
    extra_buttons = []
    ...
    def __init__(self, model, admin_site):
        super(ReimbursementAdmin, self).__init__(model, admin_site)
    ...
    def save_model(self, request, obj, form, change):
        try:
            if request.POST['export']:
                # return export_excel(request, form)
                return super(ReimbursementAdmin, self).changeform_view(request, None, '', form)
        except Exception, e:
            if obj and obj.user is None:
                obj.user = request.user
...
class ActivityAdmin(generic.BOAdmin):
    ...
    fieldsets = [
        ...
        (('other info'),{'fields':[('mail_list',),('parent',),('mail_notice','short_message_notice','weixin_notice',)],'classes': ['collapse']})
    ]
